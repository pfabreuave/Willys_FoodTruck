...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Resto del código aquí (agrupación de órdenes y creación de filas en la tabla)
...
totalFacturadoElement.textContent = `R$${totalFacturado.toFixed(2)}`;
    ...
    editButtons.forEach(button => {
      button.addEventListener('click', function() {
       ...
       //alert("recibe el numero de comanda comandaNumber  " + comandaNumber)
        resComanda = comandaNumber

        //window.location.href = `edit.html?comanda=${varComanda}`;
      });
    });
